...
func TestRouterConfiguration(t *testing.T) {
	...
	api = router.NewApiRouter(
		*orm.NewORM(gormrepository.NewRepository[Test](getDB())),
		...
		configuration.ForcedPagination(false),
	...
	//your  configuration.ForcedPagination(true) will actually override the default forced pagination value defined in configuration.DefaultConfiguration()
